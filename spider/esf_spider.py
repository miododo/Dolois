...
def getHouseInfo(html: str, detail_url: str = "") -> dict[str, str]:
    ...
    house["house_no"] = regex_pick(text, [r"(?:房屋编号|房源编号|编号)\s*[:：]?\s*([A-Za-z0-9_-]+)"])
    house["publish_time"] = regex_pick(
        text,
        [r"(?:发布时间|发布日期|发布)\s*[:：]?\s*(\d{4}[-/.年]\d{1,2}[-/.月]\d{1,2}日?)", r"(\d{4}[-/.年]\d{1,2}[-/.月]\d{1,2}日?)"],
    )
    ...
    for page in range(start_page, end_page + 1):
        ...
        for link in links:
            ...
            try:
                ...
                houses.append(parse_house_info(getHouseInfo(detail_html, detail_url=link)))
            except requests.RequestException as exc:
